...
<script type="text/javascript">
	...
	$(function(){
		...
		//为关联市场活动的模态窗口中的 搜索框绑定 敲键盘事件，目的是当判断到敲回车的时候，搜索并展现市场活动列表
		$("#aname").keydown(function(event){
			...
			if(event.keyCode==13){
				...
				//在查询并展现市场活动信息列表之后，应该将该方法强制终止（目的是为了防止触发模态窗口默认的回车事件）
				return false;
		...
		$("#bundBtn").click(function(){
			...
			//挑√了，而且有可能挑了多个√（可以执行批量关联的操作）
			}else{
				
				//alert("执行关联操作");
				
				//workbench/clue/bund.do?cid=${c.id}&aid=xxx&aid=xxx&aid=xxx
	...
	function unbund(id){
		...
		$.ajax({
			...
			data : {
				...
				"id" : id	//注意：这个参数的id是解除关联需要的关联关系表的id
