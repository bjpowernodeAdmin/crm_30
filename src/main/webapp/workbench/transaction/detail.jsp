...
<script type="text/javascript">
			...
			success : function(data){
				...
				*/
				...
				if(data.success){
					...
					//改变阶段成功
					//更新该条交易的部分信息 stage，possibility，editTime，editBy
	...
	*/
	function changeIcon(stage,i){
		...
		alert("当前阶段下标："+index);
		alert("前7个阶段后后两个阶段的分界点："+point); */
		
		//如果当前阶段的可能性为0 前7个阶段一定是黑圈 ，后两个一个是红叉一个是黑叉
		if(currentPossibility=="0"){
			...
			for(var i=0;i<point;i++){
				...
				//黑圈-----------------------------------------------------------------
				//找到span元素，移除掉原有的class（我们现在在循环中操作的span元素，使用id定位到）
				$("#"+i).removeClass();
				//为该span元素，添加新的class
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			//如果当前阶段的可能性为0  前面7个阶段的图标肯定是黑圈，后面两个阶段的图标一个是红叉一个是黑叉
			if("0".equals(currentPossibility)){
				
				//遍历所有的阶段
				for(int i=0;i<dvList.size();i++){
					
					//取得每一个遍历出来的阶段以及每一个遍历出来阶段对应的可能性
					DicValue dv = dvList.get(i);
					String listStage = dv.getValue();
					String listPossibility = pMap.get(listStage);
					
					//如果遍历出来的阶段的可能性为0  ，说明是后两个阶段，一个是红叉一个是黑叉
			...
			//如果当前阶段的可能性不为0	前面7个阶段的图标有可能 是绿圈，绿色标记，黑圈，后面两个阶段的图标一定是黑叉
			}else{
				...
				for(int i=0;i<dvList.size();i++){
					...
					//如果遍历出来的阶段的可能性为0,说明是后两个阶段，一定是黑叉
					if("0".equals(listPossibility)){
